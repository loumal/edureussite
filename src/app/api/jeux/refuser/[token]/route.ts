...
export async function GET(_req: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  await prisma.demandeJeu.update({ where: { token }, data: { statut: "REFUSE" } });

  return new NextResponse(
    htmlPage("Jeu refusé", "🚫",
      `Vous avez refusé l'accès à <strong>${demande.jeuNom}</strong>.<br>Votre enfant ne pourra pas jouer à ce jeu pour l'instant.`,
      "#ef4444"),
