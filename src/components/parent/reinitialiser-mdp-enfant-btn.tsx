...
export function ReinitialiserMdpEnfantBtn({ eleveId, prenomEnfant }: Props) {
  ...
  const reset = trpc.parent.resetMotDePasseEnfant.useMutation({
    onSuccess: () => {
      setSuccess(true);
      setPassword("");
      setConfirm("");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    reset.mutate({ eleveId, newPassword: password });
  };
  ...
  if (!open) {
    return (
      <button
        ...
        className="flex items-center gap-1.5 rounded-xl border border-[var(--color-rule)] px-3 py-1.5 text-xs font-medium text-[var(--color-ink-soft)] hover:bg-[var(--color-paper-warm)] transition-colors"
      >
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4" onClick={() => setOpen(false)}>
      ...
      >
        ...
        <p className="text-xs text-[var(--color-ink-soft)] mb-4">Pour {prenomEnfant}</p>
