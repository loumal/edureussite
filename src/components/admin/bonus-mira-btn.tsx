"use client";

import { useState } from "react";
import { trpc } from "@/lib/trpc/client";
import { useRouter } from "next/navigation";

interface Props {
  eleveId: string;
  nom: string;
}

export function BonusMiraBtn({ eleveId, nom }: Props) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [bonusMinutes, setBonusMinutes] = useState("");
  const [success, setSuccess] = useState(false);
  const [error, setError] = useState("");

  const { data: quota, isLoading } = trpc.admin.getMiraQuotaEleve.useQuery(
    { eleveId },
    { enabled: open }
  );

  const setBonus = trpc.admin.setMiraBonus.useMutation({
    onSuccess: () => {
      setSuccess(true);
      setTimeout(() => {
        setOpen(false);
        setSuccess(false);
        setBonusMinutes("");
        router.refresh();
      }, 1500);
    },
    onError: (err) => setError(err.message),
  });

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    const mins = parseInt(bonusMinutes, 10);
    if (isNaN(mins) || mins < 0 || mins > 480) {
      setError("Entre 0 et 480 minutes.");
      return;
    }
    setBonus.mutate({ eleveId, bonusMinutes: mins });
  };

  if (!open) {
    return (
      <button
        onClick={() => setOpen(true)}
        className="rounded-lg px-2 py-1 text-xs font-medium text-[var(--color-ink-soft)] hover:bg-[var(--color-paper-warm)] transition-colors"
        title="Quota Mira"
      >
        ⏱
      </button>
    );
  }

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4"
      onClick={() => setOpen(false)}
    >
      <div
        className="w-full max-w-sm rounded-2xl bg-[var(--color-paper)] p-6 shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-base font-bold text-[var(--color-ink)] mb-1">
          Quota Mira — {nom}
        </h2>

        {isLoading ? (
          <p className="text-sm text-[var(--color-ink-soft)] py-4 text-center">Chargement…</p>
        ) : quota ? (
          <>
            <div className="mb-4 rounded-xl bg-[var(--color-paper-warm)] p-3 space-y-1 text-xs text-[var(--color-ink-soft)]">
              <div className="flex justify-between">
                <span>Utilisé cette semaine</span>
                <span className="font-mono font-semibold text-[var(--color-ink)]">
                  {Math.floor(quota.secsUsed / 60)} min
                </span>
              </div>
              <div className="flex justify-between">
                <span>Quota total (base + bonus)</span>
                <span className="font-mono font-semibold text-[var(--color-ink)]">
                  {Math.floor(quota.secsMax / 60)} min
                </span>
              </div>
              <div className="flex justify-between">
                <span>Bonus actuel</span>
                <span className="font-mono font-semibold text-[var(--color-ink)]">
                  +{quota.bonusMinutes} min
                </span>
              </div>
            </div>

            {success ? (
              <p className="text-sm text-[var(--color-success)] font-medium text-center py-2">
                ✓ Bonus mis à jour
              </p>
            ) : (
              <form onSubmit={handleSubmit} className="space-y-3">
                <div>
                  <label className="text-xs font-semibold text-[var(--color-ink-soft)] block mb-1">
                    Nouveau bonus (minutes, 0 = aucun)
                  </label>
                  <input
                    type="number"
                    min={0}
                    max={480}
                    value={bonusMinutes}
                    onChange={(e) => setBonusMinutes(e.target.value)}
                    placeholder={String(quota.bonusMinutes)}
                    className="w-full rounded-xl border border-[var(--color-rule)] bg-[var(--color-paper-warm)] px-3 py-2 text-sm outline-none focus:border-[var(--color-ink)] transition-colors"
                    required
                  />
                  <p className="text-xs text-[var(--color-ink-soft)] mt-1">
                    Quota final : 30 min + bonus = {30 + (parseInt(bonusMinutes) || quota.bonusMinutes)} min/semaine
                  </p>
                </div>
                {error && <p className="text-xs text-[var(--color-accent)]">{error}</p>}
                <div className="flex gap-2 pt-1">
                  <button
                    type="submit"
                    disabled={setBonus.isPending}
                    className="flex-1 rounded-xl bg-[var(--color-ink)] px-4 py-2 text-sm font-bold text-white hover:opacity-90 disabled:opacity-50 transition-opacity"
                  >
                    {setBonus.isPending ? "…" : "Appliquer"}
                  </button>
                  <button
                    type="button"
                    onClick={() => {
                      setOpen(false);
                      setBonusMinutes("");
                      setError("");
                    }}
                    className="rounded-xl border border-[var(--color-rule)] px-4 py-2 text-sm font-medium text-[var(--color-ink-soft)] hover:bg-[var(--color-paper-warm)] transition-colors"
                  >
                    Fermer
                  </button>
                </div>
              </form>
            )}
          </>
        ) : (
          <p className="text-sm text-[var(--color-ink-soft)] py-4 text-center">
            Aucun profil élève trouvé.
          </p>
        )}
      </div>
    </div>
  );
}
